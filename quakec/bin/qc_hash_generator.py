...
def read_csv_data():
    ...
    # (Replace the whole column instead of mutating csv_data.values —
    # that only mutates a temporary copy on pandas >= 2, and cell-wise
    # assignment of ints into a str column raises on pandas >= 3.)
    first_column = csv_data.columns[0]
    hashes = []
    for original in csv_data[first_column]:
        original_lengths.append(len(original))
        original_names.append(original)
        hashes.append(int(crc16.ibm_3740(str.encode(original))))
    csv_data[first_column] = hashes
